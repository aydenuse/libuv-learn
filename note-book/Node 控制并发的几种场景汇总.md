...
背景：前不久看很多帖子、群分享面经，很多大厂面试总会遇到这类问题：
...
* 无论前后端，大批量文件传输或者大文件分片批量传输来提高效率，同时需要控制并发防止浏览器或者服务器卡死
* 后端查 `DB` ，Mongo 最为典型，三张表一起查，不推荐连表的方式；为什么，本身是个非关系型数据库，本质也是二次查询，那怎么办: `Promise.all()`,如果是晚上跑 `100w` 条数据的定时任务呢？，嗯对：`Promise.map()` 控制并发
* 心爱的女孩有很多数据需要下载，需要一个一个点点点，问你会不会爬虫，能不能爬（说起来很简单）下来？要面子的你说当然可以；嗯，你就开始学习爬虫，爬虫的过程中你发现数据量好大，或者发现自己的程序不够优雅，总是爬一点点就报错，中断，过一会，重启，又好了。聪明的你又想到了控制并发！后来，你萌顺理成章的……
...
1. 事件通知（上面用到的），
2. 任务函数 `job` 参数中传一个回调函数，等该任务执行完毕后执行回调，回调也是为了触发 " 从任务池中拿任务" 的动作（留个作业：有兴趣的同学可以试试）。
...
之前看过一些 `TCP` 限流的方案，比如 `漏桶`，`令牌桶` 等，也都是用一个 "容器" 来存储或计数的，最近看 `libuv` 源码，其中负责文件 `I/O` 的部分也是有一个 `worker` 队列，调度是由线程池去做的。在我的知识体系里，我把他们都归为一类解决问题的思想，这种问题都有一个共同的特征：

1. 有一个 "容器"：存放任务的队列(池)，或者是一个计数器
2. 什么条件下触发 "容器" 的边界限制
3. 都需要一个从 "容器" 中调度的机制
