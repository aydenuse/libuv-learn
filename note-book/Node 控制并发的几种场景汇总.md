...
今天实现这个控制并发功能：**最大并发数为 n，保证每次最多只有 n 个任务并发执行**。我会尽可能详细的描述我的思路 ~
...
初始化的时候定义一个 `任务完成时` 的事件监听，任务完成时触发：
...
1. 添加返回值，参考 `bluebird` 中  `Promise.map()` 的返回
2. 错误重试，任务失败，可以把任务重新放进任务池
3. 写成一个服务，多节点向任务池中添加任务，做成一个类似 `RabbitMQ` 的组件（嗯，`Rabbit` 的实现可能就是这个思想，这个比较有趣，有空得写一个）
...
`池` 的思想很重要，比如 `TCP` 限流就可以用 `漏桶算法`，`令牌桶算法` 来实现，`libuv` 负责文件 `I/O` 的部分也是这种思想。
